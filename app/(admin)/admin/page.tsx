 ...
 * policies de RLS para staff en `pedidos`/`usuarios`/`compras` — que,
 * igual que con las escrituras (ver `publicarMenu` en
 * `(admin)/actions.ts`), nunca se configuraron. Sin esto, un staff
 * real vería el panel vacío (RLS filtra en silencio, no da error) aun
 * con datos reales en la base.
 */
        ...
        <div className="flex flex-col items-start gap-3 rounded-card border border-warning bg-raised px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          {menuConfigurado && (
            <form
              action={async () => {
                "use server";
                await publicarMenu(anioActual, mesActualNum);
              }}
            >
