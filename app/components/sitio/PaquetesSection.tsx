...
export async function PaquetesSection() {
  ...
  const [{ data: paquetes }, { data: userData }] = await Promise.all([
    supabase
      .from("paquetes")
  ...
  // Si ya hay sesión iniciada, "Comprar paquete" va directo a /pago —
  // mandar a un usuario ya registrado por /crear-cuenta lo obliga a
  // llenar el formulario de cuenta nueva otra vez y el signUp falla
  // porque el correo ya existe.
  const autenticado = Boolean(userData.user);
  const lista = paquetes ?? [];
  const destacadoIndex = Math.floor((lista.length - 1) / 2);

  return (
    <section id="paquetes" className="flex flex-col items-start gap-14 px-6 py-16 md:px-10 lg:px-[100px] lg:py-[100px]">
      <div className="flex flex-col gap-4">
        ...
        <p className="w-full max-w-[560px] text-[17px] leading-[28px] text-muted">
        ...
        {lista.map((paquete, i) => {
          ...
          return (
            ...
            >
              ...
              <Link
                href={autenticado ? `/pago?paquete=${paquete.id}` : `/crear-cuenta?paquete=${paquete.id}`}
                className={
                  destacado
