...
/** Encabezados de columna del grid — Figma 08 (Mi calendario, node 180:2) usa el nombre completo, no la abreviatura de 3 letras. */
export const DIAS_SEMANA_LARGO = ["LUNES", "MARTES", "MIÉRCOLES", "JUEVES", "VIERNES"] as const;
...
export async function obtenerCalendarioMes(
  ...
  const [{ data: menuFijo }, { data: comodinesMesRows }, { data: pedidos }, { data: saldoRow }] =
    await Promise.all([
      ...
      // sin explicación. `menu_mes` usa anio+mes (integers), no un
      // solo campo de fecha — ver nota de esquema en types/database.ts.
      supabase
        .from("menu_mes")
        .select("dia_semana, publicado, platillos(id, nombre)")
        .eq("anio", anio)
        .eq("mes", mesNum),
      // `comodines_mes` NO es un contador de uso por usuario — es la
      // config de qué platillos son válidos como comodín este
      // anio/mes (misma forma que `menu_mes`, distinto propósito).
      supabase
        .from("comodines_mes")
        .select("platillo_id, platillos(id, nombre)")
        .eq("anio", anio)
        .eq("mes", mesNum),
      supabase
        .from("pedidos")
  ...
  // Lista de platillos válidos como comodín este mes, deduplicada.
  const comodinPlatillosMap = new Map<string, PlatilloRef>();
  for (const fila of comodinesMesRows ?? []) {
    const p = fila.platillos as unknown as PlatilloRef | null;
    if (p) comodinPlatillosMap.set(p.id, p);
  }
  const comodinPlatillos = [...comodinPlatillosMap.values()];
